...
public class UnitMenuControl : MonoBehaviour
{
    ...
    void SetHighlight()
    {
        ...
        OptionList[NumListed].transform.Find("Highlight").gameObject.SetActive(true);
    }
    ...
    {
        //Enable all options for now, but in future, only usable will become available
        ...
        foreach (Transform item in transform)
        {
            if (item.gameObject.activeSelf)
            {
                ...
                item.GetComponent<OnHighlightUI>().Control = this;
                i++;
            }
        }
        NumListed = 0;
